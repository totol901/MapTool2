...
{
	//셰이더 초기화
	return InitializeShader(device, hwnd, L"../MapTool2/shader/terrain.vs", L"../MapTool2/shader/terrain.ps");
}

bool TerrainShaderClass::BlendInitialize(ID3D11Device * device, HWND hwnd)
{
	//셰이더 초기화
	return InitializeBlendShader(device, hwnd, L"../MapTool2/shader/terrainBlend.vs", L"../MapTool2/shader/terrainBlend.ps");
}
	...
	ID3D11ShaderResourceView* texture)
{
	...
	if (!SetShaderParameters(deviceContext,
		...
		ambientolor, diffuseColor, lightDirection, texture))
	{
		return false;
	}
...
bool TerrainShaderClass::InitializeBlendShader(ID3D11Device * device, HWND hwnd, WCHAR * vsFilename, WCHAR * psFilename)
{
	HRESULT result;
	ID3D10Blob* errorMessage;
	ID3D10Blob* vertexShaderBuffer;
...
{
	RELEASE_COM(m_textureInfoBuffer);
	RELEASE_COM(m_lightBuffer);
	RELEASE_COM(m_matrixBuffer);
	RELEASE_COM(m_sampleState);
	RELEASE_COM(m_layout);
	...
	ID3D11ShaderResourceView* texture)
{
	...
	//서브 리소스(상수 버퍼 m_matrixBuffer)에 쓰기 위해 잠금
	...
	//서브 리소스(상수 버퍼 m_matrixBuffer)에 복사
...
bool TerrainShaderClass::SetBlendShaderParameters(ID3D11DeviceContext * deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, D3DXVECTOR4 ambientColor, D3DXVECTOR4 diffuseColor, D3DXVECTOR3 lightDirection)
{
	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mappedResource;
