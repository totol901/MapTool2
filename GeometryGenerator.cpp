...
void GeometryGenerator::CreateGeosphere(float radius, UINT numSubdivisions, MeshData& meshData)
{
	...
	for(UINT i = 0; i < meshData.Vertices.size(); ++i)
	{
		...
		D3DXVECTOR3 T = meshData.Vertices[i].TangentU;
		//XMStoreFloat3(&meshData.Vertices[i].TangentU, DirectX::XMVector3Normalize(T));
		D3DXVec3Normalize(&meshData.Vertices[i].TangentU, &T);
	}
}
...
void GeometryGenerator::CreateCylinder(float bottomRadius, float topRadius, float height, UINT sliceCount, UINT stackCount, MeshData& meshData)
{
	...
	for(UINT i = 0; i < ringCount; ++i)
	{
		...
		for(UINT j = 0; j <= sliceCount; ++j)
		{
			...
			D3DXVECTOR3 N;
			D3DXVec3Normalize(&N, D3DXVec3Cross(NULL, &T, &B));
			//= DirectX::XMVector3Normalize(DirectX::XMVector3Cross(T, B));
