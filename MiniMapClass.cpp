	...
	float terrainHeight)
{
	...
	result = m_MiniMapBitmap->Initialize(device, screenWidth, screenHeight, L"../MapTool2/data/colorm01.dds", 150, 150);
	if (!result)
	{
	...
	result = m_Border->Initialize(device, screenWidth, screenHeight, L"../MapTool2/data/border01.dds", 154, 154);
	if (!result)
	{
	...
	result = m_Point->Initialize(device, screenWidth, screenHeight, L"../MapTool2/data/point01.dds", 3, 3);
	if (!result)
	{
